...
def upload():
    ...
    for file in request.files.getlist('file'):
        ...
        try:
            s3_client.upload_file(os.path.join(Config.UPLOAD_FOLDER, secure_filename(f.filename)), Config.AWS_UPLOADS_BUCKET, object_name)
...
def download():
    ...
    with open(os.path.join(Config.DOWNLOAD_FOLDER, os.path.basename(bucket_path)), 'wb') as data:
