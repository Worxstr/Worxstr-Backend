...
def clock_in():
	...
	db.session \
		.query(ScheduleShift) \
		.filter(ScheduleShift.id == shift_id) \
		.update({ScheduleShift.timecard_id:timecard.id})
	timeclock = TimeClock(
		time=time_in,
		employee_id=current_user.get_id(),
...
def clock_out():
	...
	timecard_id = db.session \
		.query(TimeClock.timecard_id) \
	...
	result["need_info"] = db.session \
		.query(EmployeeInfo.need_info) \
...
def calculate_timecard(timecard_id):
	timecard = db.session \
		.query(TimeCard) \
		.filter(TimeCard.id == timecard_id) \
		.one()
	time_in = db.session \
		.query(TimeClock.time) \
		.filter(
	...
	rate = db.session \
		.query(EmployeeInfo.hourly_rate) \
...
def edit_timecard(timecard_id):
	...
	for i in changes:
		db.session \
			.query(TimeClock) \
			.filter(TimeClock.id == i["id"]) \
			.update(
				{TimeClock.time:i["time"]},
	...
	result["time_clocks"] = [timeclock.to_dict() for timeclock in db.session \
		.query(TimeClock) \
...
def get_timecards():
	...
	"""
	timecards = db.session \
	.query(TimeCard, User.first_name, User.last_name) \
	.join(User) \
	.filter(
		not TimeCard.paid,
		not TimeCard.denied,
	...
	for i in timecards:
		...
		timecard["pay_rate"] = float(db.session \
			.query(EmployeeInfo.hourly_rate) \
			.filter(EmployeeInfo.id == timecard["employee_id"]) \
			.one()[0]
		)
		timecard["time_clocks"] = [timeclock.to_dict() for timeclock in db.session. \
			query(TimeClock) \
