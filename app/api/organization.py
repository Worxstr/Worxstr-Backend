...
def edit_organization():
    ...
    db.session.query(Organization).filter(Organization.id == current_user.organization_id).update(
        {Organization.minimum_wage: org_wage}
    )

    db.session.commit()
    org = db.session.query(Organization).filter(Organization.id == current_user.organization_id).one()
    result = org.to_dict()
